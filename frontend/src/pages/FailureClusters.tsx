  ...
  return (
    <div style={{ padding: '32px', color: 'white', height: '100%', overflowY: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 600 }}>Failure Intelligence</h1>
          <p style={{ margin: '4px 0 0 0', color: '#94a3b8', fontSize: '0.875rem' }}>AI-driven root cause analysis of production errors.</p>
        </div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {mockClusters.map(cluster => (
          <div key={cluster.id} style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', overflow: 'hidden' }}>
            <div 
              onClick={() => setExpandedId(expandedId === cluster.id ? null : cluster.id)}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px', cursor: 'pointer', backgroundColor: expandedId === cluster.id ? 'rgba(255,255,255,0.01)' : 'transparent' }}
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 600 }}>{cluster.label}</h3>
                    {cluster.trending && <span style={{ backgroundColor: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}><TrendingUp size={12} /> Trending</span>}
                  </div>
                  <div style={{ color: '#94a3b8', fontSize: '0.875rem', fontFamily: 'monospace' }}>{cluster.sample_error}</div>
                </div>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{cluster.count}</div>
                  <div style={{ color: '#64748b', fontSize: '0.75rem', textTransform: 'uppercase' }}>Errors</div>
                </div>
                <div style={{ width: '80px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px', fontSize: '0.75rem', color: '#94a3b8' }}>
                    <span>Priority</span>
                    <span style={{ color: cluster.priority > 80 ? '#f43f5e' : cluster.priority > 50 ? '#f59e0b' : '#22c55e' }}>{cluster.priority}</span>
                  </div>
            ...
            {expandedId === cluster.id && (
              <div style={{ padding: '0 24px 24px 24px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ backgroundColor: 'rgba(79, 110, 247, 0.05)', border: '1px solid rgba(79, 110, 247, 0.2)', borderRadius: '8px', padding: '16px', marginTop: '24px', marginBottom: '24px', display: 'flex', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#e2e8f0', fontSize: '0.875rem', lineHeight: 1.5 }}>{cluster.root_cause}</div>
                  </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>Sample Traces</h4>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                  <tbody>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '12px', color: '#4f6ef7', cursor: 'pointer' }}>run-38491</td>
                      <td style={{ padding: '12px', color: '#94a3b8' }}>gpt-4o</td>
                      <td style={{ padding: '12px', color: '#94a3b8', textAlign: 'right' }}>10 mins ago</td>
                    </tr>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '12px', color: '#4f6ef7', cursor: 'pointer' }}>run-38482</td>
                      <td style={{ padding: '12px', color: '#94a3b8' }}>gpt-4o</td>
                      <td style={{ padding: '12px', color: '#94a3b8', textAlign: 'right' }}>15 mins ago</td>
                    </tr>
                  </tbody>
                </table>
              </div>
